...
This is a comprehensive metrics object generated by the online voice analysis system (online-praat-analysis Lambda):

*   **`sustained` (Map)**: Sustained vowel test metrics
    *   `f0_mean` (Number): Mean fundamental frequency in Hz
    *   `f0_sd` (Number): Standard deviation of F0
    *   `mpt_s` (Number): Maximum phonation time in seconds
    *   `spl_dbA_est` (Number): Estimated sound pressure level in dB(A)
    *   `jitter_local_percent` (Number): Local jitter percentage
    *   `shimmer_local_percent` (Number): Local shimmer percentage
    *   `hnr_db` (Number): Harmonics-to-noise ratio in dB
    *   `formants_low` (Map): Formants at low pitch `{ f0_mean, F1, F2, F3, spl_dbA_est }`
    *   `formants_high` (Map): Formants at high pitch `{ f0_mean, F1, F2, F3, spl_dbA_est }`

*   **`vrp` (Map)**: Voice range profile metrics
    *   `f0_min` (Number): Minimum F0 in Hz
    *   `f0_max` (Number): Maximum F0 in Hz
    *   `spl_min` (Number): Minimum SPL in dB
    *   `spl_max` (Number): Maximum SPL in dB
    *   `bins` (List): Array of VRP bin data `[{ semi, f0_center_hz, count, spl_min, spl_max, spl_mean }]`

*   **`reading` (Map)**: Reading passage metrics
    *   `f0_mean` (Number): Mean F0 during reading
    *   `f0_sd` (Number): F0 standard deviation
    *   `f0_stats` (Map): F0 statistics `{ median, p10, p90 }`
    *   `duration_s` (Number): Reading duration in seconds
    *   `voiced_ratio` (Number): Ratio of voiced speech
    *   `pause_count` (Number): Number of pauses detected

*   **`spontaneous` (Map)**: Spontaneous speech metrics (same structure as `reading`)
    *   `f0_mean`, `f0_sd`, `f0_stats`, `duration_s`, `voiced_ratio`, `pause_count`

*   **`questionnaires` (Map)**: Voice-related questionnaire scores
    *   `RBH` (Map): RBH perceptual rating `{ R, B, H }` (Numbers 0-3)
    *   `OVHS-9 Total` (Number): Overall Voice Handicap Scale score
    *   `TVQ-G Total` (Number): Trans Voice Questionnaire total score
    *   `TVQ-G Percent` (String): TVQ percentage score (e.g., "35%")
