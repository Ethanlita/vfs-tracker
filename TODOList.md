...
40. ~~~解决检测限（也就是测不出特定共振峰时把F2当做F1）的问题（已解决）~~~
41. ~~~增加一个对持续元音的Formant-SPL分析，在分析共振峰的步骤中，使用在第二步中录制的持续元音，选择Jitter和Shimmer最小的录音进行分析（已解决）~~~
42. ~~~更新多录音选择标准：（已解决）~~~
    1. ~~~对于测量MPT任务，选择最长的录音（已解决）~~~
    2. ~~~对于测量基频任务，选择基频最高的录音（已解决）~~~
    3. ~~~对于上滑音，选择最高基频最高的录音（已解决）~~~
    4. ~~~对于下滑音，选择最低基频最低的录音（已解决）~~~
43. ~~~对于MPT测量，不能直接获取音频长度，而是要计算其中有效音频的长度（去掉用户没出声部分）（已解决）~~~
